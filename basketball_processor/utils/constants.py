...
def get_conference_for_date(
    ...
    if year:
        try:
            ...
            historical_conf = get_conference_for_school(team_name, year, gender)
            if historical_conf:
                return historical_conf
            # Also try canonical name
            if canonical != team_name:
                historical_conf = get_conference_for_school(canonical, year, gender)
                if historical_conf:
                    return historical_conf
        except ImportError:
...
TEAM_ALIASES = {
    ...
    # === ESPN-specific Name Variations ===
    # ESPN often uses abbreviated names different from Sports Reference
    ...
    "Hawai'i": 'Hawaii',  # Standard apostrophe
    "Hawai\u2019i": 'Hawaii',  # ESPN uses right single quotation (U+2019)
    ...
    'SDSU': 'South Dakota State',  # Context: Summit League
    ...
    'TAM-CC': 'Texas A&M-Corpus Christi',
...
# === ESPN PLAY-BY-PLAY MILESTONE CONFIGS ===
